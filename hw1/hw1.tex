...
\end{subparts}
    ...
    \begin{checkboxes}
        % YOUR ANSWER
        % Change \choice to \CorrectChoice for the appropriate selection/selections 
        \CorrectChoice $\sum_{y=1}^n y \frac{1}{2^y}$
...
\begin{subparts}
    ...
    \begin{checkboxes}
        ...
        \CorrectChoice The inner product $\mathbf{w^T(x' - x'')}$ is 1
...
\subpart[3] Fill in the blanks to complete the above function.
    ...
    % Put your solution in the your_code_solution environment
    \begin{your_code_solution}
    min(minCostPath(grid[r+1][c]), minCostPath(grid[r][c+1]))
    \end{your_code_solution}
    \end{your_code_solution_outer}
...
1. I collaborated with Grace Chung on multiple Linear Algebra questions. We searched the text books and other provided materials along with the internet to refresh our minds on formulas like the determinant, rank, trace, and a matrix being invertible. She also explained to me what problem 3.8 was asking.

2. I explained to Grace my understanding of how to read joint probability tables. Also we graphed some functions and I showed her how some grew faster than others, which meant they were the tighter bound. In other words, if g(x) > f(x) for all x, then f(x) belongs to O(g(x)).

3. No. Didn't try to look for it
\end{your_solution}
